  ...
  return (
    <main>
      ...
      {demos.length > 0 && <section className="px-6 py-16 sm:py-20"><div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">{demos.map((demo) => <Link key={demo.id} href={`/report/${demo.id}`} className="border border-[var(--border)] bg-[var(--card)] p-6 transition hover:-translate-y-1 hover:border-[var(--teal)]"><span className="text-xs uppercase tracking-widest text-[var(--teal-dark)]">{demo.question_type.replace('_', ' ')}</span><h3 className="mt-5 text-2xl">{demo.subject_name}</h3><p className="mt-3 line-clamp-2 text-sm leading-6 text-[var(--muted)]">{demo.element_profile}</p><span className="mt-6 inline-block text-sm font-semibold text-[var(--teal-dark)] underline underline-offset-4">Read report →</span></Link>)}</div>
      </div></section>}
    </main>
  );
}
