  ...
  return (
    <main className="flex flex-col">
      ...
      {demos.length > 0 && <section className="px-5 py-8 sm:px-6 sm:py-12"><div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 grid gap-3 sm:mt-6 sm:gap-4 md:grid-cols-3">{demos.map((demo) => <Link key={demo.id} href={`/report/${demo.id}`} className="border border-[var(--border)] bg-[var(--card)] p-4 transition hover:-translate-y-1 hover:border-[var(--teal)] sm:p-5"><span className="text-xs uppercase tracking-widest text-[var(--teal-dark)]">Sample summary</span><h3 className="mt-2 text-xl sm:mt-3 sm:text-2xl">{demoDisplayName(demo.id, demo.subject_name)}</h3><p className="mt-1 line-clamp-2 text-sm leading-6 text-[var(--muted)] sm:mt-2">Current Day Master report format</p><span className="mt-3 inline-block text-sm font-semibold text-[var(--teal-dark)] underline underline-offset-4 sm:mt-4">Read report →</span></Link>)}</div>
      </div></section>}
    </main>
  );
}
