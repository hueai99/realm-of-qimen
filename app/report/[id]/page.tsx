  ...
  return <main className="min-h-screen">
    ...
    <article className="mx-auto max-w-5xl px-6 pb-20 pt-8">
      ...
      {summary ? <div className="mt-12 space-y-12">
        ...
        <PointSection title={`Small ways to support ${report.subject_name}`} points={summary.parenting_tips} guidanceOnly />
...
function PointSection({ title, points, guidanceOnly = false }: { title: string; points: { heading: string; body: string; guidance?: string }[]; guidanceOnly?: boolean }) {
  return <section><h2 className="text-3xl">{title}</h2><div className="mt-6 grid gap-5 md:grid-cols-3">{points.map((point) => <div key={point.heading} className="border border-[#d7cbbd] bg-[#fffaf0] p-6"><h3 className="text-xl">{point.heading}</h3>{!guidanceOnly && <p className="mt-3 leading-7 text-[#665a50]">{point.body}</p>}{(point.guidance || guidanceOnly) && <div className={`${guidanceOnly ? "mt-3" : "mt-5 border-t border-[#e2d7ca] pt-4"} flex gap-3 leading-7 text-[#665a50]`}><GuidanceIcon /><p>{point.guidance ?? point.body}</p></div>}</div>)}</div></section>;
}
