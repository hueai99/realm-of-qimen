...
export default function ReportFeedback({ reportId, childName, initialEmail, emailToken }: Props) {
  ...
  if (submitted) return <section className="border border-[#d7cbbd] bg-[#fffaf0] p-6 text-center sm:p-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#9b3c2b]">Thank you</p><h2 className="mt-2 text-2xl">Your feedback helps us improve.</h2>{emailCopy && <p className="mt-2 text-[#665a50]">Your request for an emailed copy has been received.</p>}</section>;
  ...
  return <section className="border border-[#d7cbbd] bg-[#fffaf0] p-6 sm:p-8">
    ...
    <form onSubmit={submit} className="mt-6">
      <fieldset className="grid gap-3 sm:grid-cols-2"><legend className="sr-only">Summary accuracy</legend>{choices.map(choice=><label key={choice.value} className={`flex cursor-pointer items-center gap-3 border p-4 transition ${rating===choice.value?"border-[#9b3c2b] bg-[#f7eee3]":"border-[#d7cbbd] bg-[#fffdf8]"}`}><input type="radio" name="accuracy" value={choice.value} checked={rating===choice.value} onChange={()=>{setRating(choice.value);setError("")}} className="h-4 w-4 accent-[#9b3c2b]"/><span>{choice.label}</span></label>)}</fieldset>
      <label className="mt-6 block">Anything you would like us to know?<textarea value={comment} onChange={e=>setComment(e.target.value)} maxLength={1000} rows={4} placeholder="Tell us what felt accurate, unclear, or missing." className="mt-2 w-full resize-y border border-[#d7cbbd] bg-[#fffdf8] px-4 py-3 outline-none focus:border-[#9b3c2b]"/></label>
      <div className="mt-5 space-y-4 border-t border-[#e2d7ca] pt-5 leading-7">
        {emailToken&&<label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={emailCopy} onChange={e=>setEmailCopy(e.target.checked)} className="mt-1 h-4 w-4 accent-[#9b3c2b]"/><span>Email me a copy of this Bazi summary.</span></label>}
        <label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={interestedInMore} onChange={e=>setInterestedInMore(e.target.checked)} className="mt-1 h-4 w-4 accent-[#9b3c2b]"/><span>I would like to learn more about what Bazi can reveal about my child.</span></label>
      </div>
      {error&&!confirmEmail&&<p role="alert" className="mt-4 text-sm text-[#8a4b3c]">{error}</p>}
    ...
    {confirmEmail&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-[#211b16]/75 px-5 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="confirm-email-title"><div className="w-full max-w-lg bg-[#fffaf0] p-7 shadow-2xl sm:p-9"><h2 id="confirm-email-title" className="text-3xl">Confirm your email</h2><p className="mt-3 leading-7 text-[#665a50]">Please check that this is the best email address to use.</p><label className="mt-6 block text-sm">Email address<input type="email" required value={email} onChange={e=>{setEmail(e.target.value);setError("")}} className="mt-2 w-full border border-[#c9bcad] bg-[#fffdf8] px-4 py-3 outline-none focus:border-[#9b3c2b]"/></label>{error&&<p role="alert" className="mt-5 text-sm text-[#8a4b3c]">{error}</p>}<div className="mt-7 flex gap-3"><button type="button" disabled={busy||!email} onClick={()=>void save(email)} className="bg-[#9b3c2b] px-6 py-3 font-semibold text-white disabled:opacity-50">{busy?"Sending...":"Confirm and continue"}</button><button type="button" disabled={busy} onClick={()=>{setConfirmEmail(false);setError("")}} className="border border-[#c9bcad] px-6 py-3">Back</button></div></div></div>}
  </section>;
}
