...
export default function IntakeForm() {
  ...
  return <form action={submit} className="rounded-sm border border-[#d7cbbd] bg-[#fffaf0] p-6 shadow-[0_20px_50px_rgba(71,49,32,.09)] sm:p-9">
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <label className="text-sm">Country of birth<select required name="birth_country" value={birthCountry} onChange={(event) => { const country = event.target.value; const code = countries.find(([name]) => name === country)?.[1] ?? "+"; setBirthCountry(country); setPhoneCode(`${country}|${code}`); }} className={cls}>{countries.map(([country]) => <option key={country} value={country}>{country}</option>)}</select></label>
      ...
      <div className="w-full max-w-2xl rounded-sm bg-[#fffaf0] p-7 shadow-2xl sm:p-10">
        <div>
          ...
          <div className="mt-7 border-t border-[#d7cbbd] pt-6 text-center">
            ...
            <p className={`${busy ? "mt-4" : ""} text-xs font-bold uppercase tracking-[.22em] text-[#9b3c2b]`}>{busy ? "Preparing report" : "Report prepared"}</p>
...
function SampleBaziChart() {
  ...
  return <figure className="mt-6"><figcaption className="mb-3 text-center text-xs font-bold uppercase tracking-[.18em] text-[#9b3c2b]">Sample Bazi chart</figcaption><div className="grid grid-cols-4 gap-px overflow-hidden border border-[#d7cbbd] bg-[#d7cbbd]">{sample.map(([label, stem, branch]) => <div key={label} className="bg-[#fffdf8] px-2 py-3 text-center"><p className="text-[10px] uppercase tracking-wider text-[#877b70]">{label}</p><p className="mt-2 text-2xl">{stem}</p><p className="mt-1 text-2xl">{branch}</p></div>)}</div></figure>;
}
