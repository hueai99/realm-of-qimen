...
export default function IntakeForm() {
  ...
  return <form onInput={() => error && setError("")} onSubmit={(event) => { event.preventDefault(); void submit(new FormData(event.currentTarget)); }} className="rounded-sm border border-[var(--border)] bg-[var(--card)] p-6 shadow-[0_20px_50px_rgba(71,49,32,.08)] sm:p-9">
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <fieldset className="text-sm"><legend>Date of birth</legend><div className="grid grid-cols-3 gap-2"><select required name="birth_day" defaultValue="" aria-label="Birth day" className={`${cls} px-2`}><option value="" disabled>DD</option>{Array.from({length:31},(_,i)=><option key={i+1} value={String(i+1).padStart(2,"0")}>{String(i+1).padStart(2,"0")}</option>)}</select><select required name="birth_month" defaultValue="" aria-label="Birth month" className={`${cls} px-2`}><option value="" disabled>MMM</option>{months.map((month,i)=><option key={month} value={String(i+1).padStart(2,"0")}>{month}</option>)}</select><select required name="birth_year" defaultValue="" aria-label="Birth year" className={`${cls} px-2`}><option value="" disabled>YYYY</option>{Array.from({length:100},(_,i)=>thisYear-i).map(year=><option key={year} value={year}>{year}</option>)}</select></div></fieldset>
