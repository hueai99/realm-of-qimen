...
export async function POST(request: Request) {
  ...
  const subject_name = normaliseName(body.subject_name ?? ""); const email = body.email?.trim().toLowerCase(); const phone = body.phone?.trim(); const birth_date = body.birth_date || `${body.birth_year}-${body.birth_month}-${body.birth_day}`; const birth_time = body.birth_time || null; const birth_place = body.birth_place?.trim(); const parenting_concern = body.parenting_concern?.trim() || null; const gender = body.gender; const question_type = body.question_type as QuestionType;
  if (!subject_name || subject_name.length > 80 || !email || !/^\S+@\S+\.\S+$/.test(email) || !phone || !/^[+()\d\s-]{7,30}$/.test(phone) || !/^\d{4}-\d{2}-\d{2}$/.test(birth_date) || !/^\d{2}:\d{2}$/.test(birth_time ?? "") || !birth_place || birth_place.length > 120 || (parenting_concern?.length ?? 0) > 600 || !allowed.has(question_type) || !["male","female","other"].includes(gender)) return NextResponse.json({ error: "Please check all required fields." }, { status: 422 });
