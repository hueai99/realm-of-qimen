...
export async function POST(request: Request) {
  ...
  const subject_name = body.subject_name?.trim(); const email = body.email?.trim().toLowerCase(); const birth_date = body.birth_date; const birth_time = body.birth_time || null; const birth_place = body.birth_place?.trim(); const parenting_concern = body.parenting_concern?.trim() || null; const gender = body.gender; const question_type = body.question_type as QuestionType;
