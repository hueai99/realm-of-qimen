...
export async function POST(request: Request) {
  ...
  let { data: report, error } = await db.from("bazi_reports").insert({ subject_name, parent_name, email, birth_date, birth_time, birth_place, parenting_concern, gender, question_type }).select("id").single();
  if (error && /parent_name/i.test(`${error.message} ${error.details ?? ""}`)) {
    ({ data: report, error } = await db.from("bazi_reports").insert({ subject_name, email, birth_date, birth_time, birth_place, parenting_concern, gender, question_type }).select("id").single());
  }
  if (error || !report) {
  ...
  let { error: leadError } = await db.from("leads").insert({ name: parent_name, parent_name, email, phone, report_id: report.id, conversion_status: "new" });
  if (leadError && /parent_name/i.test(`${leadError.message} ${leadError.details ?? ""}`)) {
    ({ error: leadError } = await db.from("leads").insert({ name: parent_name, email, phone, report_id: report.id, conversion_status: "new" }));
  }
  if (leadError) { console.error("Lead insert failed", { code: leadError.code, message: leadError.message, details: leadError.details, hint: leadError.hint }); return NextResponse.json({ error: "Report saved, but the contact record could not be created." }, { status: 500 }); }
  return NextResponse.json({ report_id: report.id, email_action_token: createEmailActionToken(report.id) }, { status: 201 });
}
