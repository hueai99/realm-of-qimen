...
function groundedSummary(name: string, dayMasterName: string, dayMaster: string, strength: "Strong" | "Weak", concern?: string | null): SummaryReport {
  ...
  const seasonLink = `In Bazi, the ${dayMaster} Day Master is pictured as ${profile.image}. For ${name}, this image offers a gentle way to understand ${profile.warmIntroduction}. ${support}`;
  return {
    personality: `${seasonLink} A parent may recognise it in small moments: the way ${name} enters a new situation, holds onto something important, or finds a way back after disappointment. The chart provides the image; daily life reveals how that image belongs uniquely to this child.`,
    strengths: profile.strengths.map((point, index) => ({ heading: point.heading, body: index === 0
      ? `${name} ${point.meaning}. You may catch a glimpse of it when ${point.everyday}. When it appears, ${point.support}. These small moments often say more about what matters to ${name} than a big achievement ever could.`
      : index === 1
        ? `Another side of ${name} may show itself when ${point.everyday}. Beneath that moment is a child who ${point.meaning}. A helpful response is simple: ${point.support}. This gives the strength room to grow without turning it into another expectation.`
        : `Sometimes the clearest sign is ${point.everyday}. In those moments, ${name} ${point.meaning}. Parents can nurture this by remembering to ${point.support}. There is no need to force it; being warmly noticed is often enough to help the child trust this part of himself or herself.`, basis: { factor: "Day Master", value: `${dayMasterName} / ${strength}` } })),
    soft_spots: profile.softSpots.map((point, index) => ({ heading: point.heading, body: index === 0
      ? `On harder days, ${name} ${point.meaning}. It may show up as ${point.everyday}, especially when energy is already running low. In that moment, ${point.support}. Feeling understood first can make the next step much easier to accept.`
      : `Parents may also notice moments when ${point.everyday}. This can be ${name}'s way of showing that he or she ${point.meaning}. It often helps when adults ${point.support}. A little warmth and breathing room can turn a tense moment into a chance to reconnect.`, basis: { factor: "Day Master expression", value: `${dayMasterName} / ${strength}` } })),
    ...
    closing_encouragement: `${name} is ${profile.closing}. No chart can contain everything a parent already knows through love, attention, and daily life. When the adults around ${name} stay curious about what sits beneath a difficult moment—and notice the small signs of growth as warmly as the big ones—the child receives room to become more fully himself or herself.`,
  };
}
...
export async function generateReading(input: Input): Promise<Reading> {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", { method: "POST", signal: AbortSignal.timeout(6000), headers: { "content-type": "application/json", authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: JSON.stringify({ model: process.env.OPENAI_MODEL ?? "gpt-4o-mini", response_format: { type: "json_object" }, temperature: 0.55, messages: [{ role: "system", content: "Return JSON containing only report_content. Write like an experienced, caring Bazi consultant speaking personally to one parent after listening carefully. The prose should feel alive, warm, and worth reading aloud—not like a textbook, checklist, brochure, or AI summary. Use only the supplied reviewed Day Master guidance and verified strong/weak state. Do not introduce traits outside that guidance. Keep calculation details internal because report scope is explained separately on the page. Open with the Day Master's simple natural image, then connect it to this particular child through familiar, emotionally recognisable moments. Use short, clear sentences alongside occasional longer reflective ones. Prefer concrete words over abstractions. Avoid convoluted clauses, generic disclaimers, repeated conclusions, and stock phrases attached to every card. Never say 'rather than treating it as a flaw', 'this quality may not appear in every setting', or similar defensive wording. Do not repeat 'Day Master' in every card. Use the child's name and selected he/she pronouns. Strengths should feel specific and affirming. Soft spots should help a parent see what may sit underneath the behaviour, without sounding negative. Write exactly five parenting tips. Each tip must be 45-75 words and include why it may help, a realistic example or phrase a parent could use, and reassurance rather than commands. Parenting suggestions are options to try, not facts guaranteed by the chart. Use cautious phrases such as 'may', 'often', and 'parents may notice', but vary them naturally. Present Bazi as a reflective framework, not science, religion, prediction, diagnosis, or fixed destiny. Age, environment, experience, and choice affect how any tendency appears. Never invent structures, profile stars, Ten Gods, or other chart factors. Never identify internal sources or tools. report_content must contain personality, exactly 3 strengths, 2-3 soft_spots, exactly 5 parenting_tips, optional concern_response, and closing_encouragement; each list item has heading and a substantial, natural body." }, { role: "user", content: JSON.stringify({ child: input, verified_chart: verified.chart_data, reviewed_day_master_guidance: getDayMasterKnowledge(calculatedChart.day_master_name ?? "Gui"), fixed_element_profile: verified.element_profile }) }] }) });
