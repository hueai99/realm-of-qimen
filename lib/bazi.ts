...
function groundedSummary(name: string, dayMasterName: string, dayMaster: string, strength: "Strong" | "Weak", concern?: string | null): SummaryReport {
  ...
  const seasonLink = `${name}, born under the ${dayMaster} Day Master, may carry ${profile.warmIntroduction}. The image traditionally associated with this Day Master is ${profile.image}; it is a way of describing a natural rhythm, not placing the child in a box. ${support}`;
  return {
    ...
    strengths: profile.strengths.map((point) => ({ heading: point.heading, body: `${name} ${point.meaning}. Parents may recognise this in an ordinary moment, such as ${point.everyday}. It can help to ${point.support}. The quality may not appear in every setting or every mood, but that does not make it any less meaningful when it does.`, basis: { factor: "Day Master", value: `${dayMasterName} / ${strength}` } })),
    soft_spots: profile.softSpots.map((point) => ({ heading: point.heading, body: `${name} ${point.meaning}. This may look like ${point.everyday}, particularly when the child is tired, rushed, or unsure. Rather than treating it as a flaw, parents can ${point.support}. A warm response protects connection while the child learns a steadier way through the moment.`, basis: { factor: "Day Master expression", value: `${dayMasterName} / ${strength}` } })),
