...
function concernWeeklyAction(concern: string, name: string): { situation: string; action: string; phrase: string; sign: string } {
  ...
  return { situation: "Test one small change for seven days", action: `Choose one part of the concern that can be observed. Try the same supportive approach for a week before deciding whether it helped.`, phrase: "Let us try this for one week, then talk about what felt different.", sign: `${name} responds more openly, or you gain a clearer sense of what support is useful.` };
}
