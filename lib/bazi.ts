...
function groundedSummary(name: string, dayMaster: string, element: string, strength: "Strong" | "Weak", _season: string, _seasonalStateName: string, concern?: string | null): SummaryReport {
  ...
  const support = strength === "Weak"
    ? `${name}'s Day Master is less supported by the birth season. This does not make the child weak. It can simply mean that these qualities emerge more gently, especially somewhere new or when expectations feel heavy. Time to settle, a clear sense of what comes next, and permission to learn through mistakes can make a noticeable difference.`
    : `${name}'s Day Master is well supported by the birth season, so these qualities may be easier to notice. At their best, they give the child a natural source of confidence and momentum. On a difficult day, the same energy may need a calm boundary or a useful direction rather than being pushed down.`;
...
export async function generateReading(input: Input): Promise<Reading> {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/chat/completions", { method: "POST", signal: AbortSignal.timeout(6000), headers: { "content-type": "application/json", authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: JSON.stringify({ model: process.env.OPENAI_MODEL ?? "gpt-4o-mini", response_format: { type: "json_object" }, temperature: 0.45, messages: [{ role: "system", content: "Return JSON containing only report_content. Write like an experienced, caring Bazi consultant speaking personally to a parent after listening carefully—not like a textbook, checklist, or AI summary. Ground every interpretation in the supplied verified Day Master and its season-based strong/weak state. Strength means how supported the Day Master energy is, never character weakness. Explain that distinction naturally once in the personality section; do not repeat 'Day Master' in every card. Use the child's name and selected he/she pronouns. Elaborate each point with a recognisable home, school, friendship, transition, mistake, or emotional moment, followed by gentle meaning or support. Vary sentence shapes and avoid generic filler. Strengths should feel specific and affirming; soft spots should show empathy and never label a flaw. Parenting suggestions are options to try, not facts guaranteed by the chart. Use cautious phrases such as 'may', 'often', and 'parents may notice'. Present Bazi as a reflective framework, not science, religion, prediction, diagnosis, or fixed destiny. Age, environment, experience, and choice affect how any tendency appears. Never invent structures, profile stars, Ten Gods, or other chart factors. Never identify internal sources or tools. report_content must contain personality, exactly 3 strengths, 2-3 soft_spots, 2-3 parenting_tips, optional concern_response, and closing_encouragement; each list item has heading and a substantial, natural body." }, { role: "user", content: JSON.stringify({ child: input, verified_chart: verified.chart_data, fixed_element_profile: verified.element_profile }) }] }) });
