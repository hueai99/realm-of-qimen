...
export async function sendSummaryEmail(input: { reportId: string; email: string; parentName: string; childName: string; summary: SummaryReport }) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html: `<div style="font-family:Georgia,serif;max-width:620px;margin:auto;color:#272727;line-height:1.65"><p style="color:#007789;font-size:12px;letter-spacing:2px;text-transform:uppercase">Realm of Qimen</p><h1>${escapeHtml(input.childName)}'s Bazi personality summary</h1><p>Dear ${escapeHtml(input.parentName)},</p>${paragraphs(input.summary.personality)}<h2>Top 3 strengths</h2>${reportPoints(input.summary.strengths)}<h2>Where support may help</h2>${reportPoints(input.summary.soft_spots)}${actionHtml}<h2>Closing encouragement</h2>${paragraphs(input.summary.closing_encouragement)}<p><a href="${reportUrl}" style="display:inline-block;background:#007789;color:white;padding:12px 22px;text-decoration:none">View the online summary</a></p></div>`,
    }),
  });
